...
(async () => {
  ...
  try {
    ...
    const activePage = id => js(`document.querySelector('.organizer-page-row.active')?.dataset.pageId===${JSON.stringify(id)}`);
    ...
    await until(() => js('Boolean(document.querySelector(".home-view"))'), 'Home did not open');
    window.emit('app-command', {}, 'browser-backward');
    await until(() => activePage(otherId), 'Native Back did not return to second page');
    await thumb(3);
    await until(() => activePage(identity.noteId), 'Mouse Back did not return to first page');
    await thumb(4);
    await until(() => activePage(otherId), 'Mouse Forward did not return to second page');
    await js(`Array.from(document.querySelectorAll('.sidebar button')).find(b=>b.textContent==='Tags').click()`);
    await until(() => js('Boolean(document.querySelector(".tags-view"))'), 'Tags did not open');
    await thumb(4);
    assert.ok(await js('Boolean(document.querySelector(".tags-view"))'), 'Forward branch should be discarded');
    window.emit('app-command', {}, 'browser-backward');
    await until(() => activePage(otherId), 'Native Back did not leave Tags');
    window.emit('app-command', {}, 'browser-backward');
