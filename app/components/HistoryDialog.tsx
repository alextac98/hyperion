...
export function HistoryDialog({ vaultId, noteId, onClose }: { vaultId: string; noteId?: string; onClose: () => void }) {
  ...
  const reload = async () => {
    ...
    setVersions(result);
    if (result[0]) setSelected(await requireDataService().repositoryExecute<PageRevision>({ operation: "getRevision", vaultId, revisionId: result[0].id }));
  };
  ...
  return <Dialog label="Page history" className="history-layer" onClose={onClose} busy={working}><section className="history-dialog">
    ...
    {noteId && <form onSubmit={event => { event.preventDefault(); void run(async () => { await dataOperation(() => requireDataService().repositoryExecute({ operation: "captureRevision", vaultId, noteId, label: label.trim() })); setLabel(""); await reload(); }); }}><input aria-label="Version name" placeholder="Name this version…" value={label} onChange={e => setLabel(e.target.value)} /><button disabled={working || !label.trim()}>Save named version</button></form>}
    {error && <p role="alert" className="data-error">{error}</p>}
    <div className="history-body"><nav aria-label="Saved versions">{versions.length === 0 && <p>No versions yet. Automatic history is captured every minute when pages change.</p>}{versions.map(version => <button key={version.id} className={selected?.id === version.id ? "selected" : ""} disabled={working} onClick={() => void run(async () => { setSelected(await requireDataService().repositoryExecute<PageRevision>({ operation: "getRevision", vaultId, revisionId: version.id })); })}><strong>{version.label || "Automatic version"}</strong>{!noteId && <span>{version.note.title}</span>}<time>{new Date(version.createdAt).toLocaleString()}</time></button>)}</nav><div className="history-preview">{selected && <div className="history-metadata"><strong>{selected.note.title}</strong><span>{selected.note.tags.join(" · ")}</span></div>}{previewLoading && <p>Opening version…</p>}<div ref={mount} /></div></div>
