...
import { requireDataService, requireDesktop, platformRuntime } from "../platform/runtime";
...
export function DataRecovery({ onHistory }: { onHistory: () => void }) {
  ...
  const refresh = () => requireDataService().listBackups().then(setBackups);
  ...
  return <div className="data-recovery"><h3>Backups and history</h3><p>Automatic backups run at startup and daily while Hyperion is open. The latest 10 are retained. Manual backups are kept.</p><div className="recovery-actions"><button disabled={busy} onClick={() => void run(async () => { const backup = await dataOperation(() => requireDataService().createBackup()); setMessage(`Backup verified: ${backup.path}`); await refresh(); })}>Back up now</button>{platformRuntime.capabilities.configurableStorage && <><button disabled={busy} onClick={() => void run(() => requireDesktop().showBackupFolder())}>Open backup folder</button><button disabled={busy} onClick={() => void run(async () => { const result = await dataOperation(() => requireDesktop().restoreBackup()); if (result) setMessage(`Backup restored to ${result.directory}. Choose Open existing vault from the vault menu to open it. Close the current vault first if it is another version of the same vault. Your current vault remains active.`); })}>Restore database backup…</button></>}<button onClick={onHistory}>Browse page history</button></div><p>{backups.length} database backups available. Copy backups to another device or drive to protect against disk loss.</p>{message && <p role="status">{message}</p>}{error && <p className="data-error" role="alert">{error}</p>}</div>;
}
