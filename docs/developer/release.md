...
Release notes begin with platform-specific installer links generated by
`scripts/release-downloads.mjs`. The workflow excludes DMG blockmaps, retains ZIP
and EXE blockmaps and update manifests, and generates checksums for the published files.

The app checks for updates 15 seconds after startup and every four hours. Users
can also check in Settings → Updates. Downloads require an explicit action, show
progress in the sidebar and Settings, and offer retry/manual download on failure.
Downloaded updates install only after Restart to update; ordinary quitting does
not install them. Installation waits for the renderer to finish saving.

## Preview updates before publishing

Run `pnpm dev:updates` for a simulated update in the real desktop UI. Each launch
creates a separate temporary profile and data folder, printed in the terminal.
Your usual Hyperion data is not used. The UI labels the preview as simulated.

- `pnpm dev:updates -- download-error`: first download fails; Retry succeeds.
- `pnpm dev:updates -- check-error`: first check fails; Retry succeeds.
- `pnpm dev:updates -- up-to-date`: no newer version is available.

The preview checks after two seconds. Try the sidebar control and Settings →
Updates. Restart to update waits for saving and relaunches the preview showing
the simulated new version; it does not install or replace application files.
The temporary profile is retained across that restart. Close each preview when
finished. Preview mode is disabled in packaged builds.

`pnpm test:updates` automates the available, progress, error, retry, and ready UI
